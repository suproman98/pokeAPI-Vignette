...
The three starters are present (Blaziken, Sceptile, and Swampert), with Rayquaza being the strongest legendary and pokemon overall. There are a lot higher total stat numbers with this generation than in Kanto, but the craziest find was that Slaking, a very common pokemon, has the second highest total stat count. Let's look at the `totalstat` breakdown across all these pokemon.
```{r hoenn type}
...
Again, the three starters and many of the legendaries are present. I have half a mind to not count `Arceus` since it is the literal god of pokemon and can technically be any single type of pokemon. Many of the pokemon on this list have unique typing or are pseudo/actual legendary pokemon. With the even smaller pool of pokemon present that are unique to this generation, it will be a bit tougher to analyze type. Let's look at the `totalstat` spread.
```{r sinnoh type}
